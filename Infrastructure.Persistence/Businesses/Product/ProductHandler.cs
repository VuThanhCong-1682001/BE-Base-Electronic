        ...
        public async Task<Response> Create(ProductBaseModel model)
        {
            try
            {
                ...
                if (model.ProductColors != null && model.ProductColors.Count > 0)
                {
                    ...
                    {
                        var categoryModel = new ProductColor();

                        categoryModel.Id = Guid.NewGuid();
                        categoryModel.Status = true;
                        categoryModel.ColorId = item.ColorId;
            ...
            try
            {
                ...
                foreach (var item in listFilter)
                {
                    ...
                    item.ProductColors = (from a in _dataContext.ProductColors.Where(x => x.ProductId == item.Id).ToList()
                        join b in _dataContext.Colors on a.ColorId equals b.Id
                        select new ProductColorModel()
                        {
                ...
                {
                    listFilter = listFilter.Where(x => x.ProductColors.Any(c => c.ColorId == filter.ColorId)).ToList();
                }
            ...
            try
            {
                ...
                if (data != null)
                {
                    ...
                    data.ProductColors = (from a in _dataContext.ProductColors.Where(x => x.ProductId == data.Id).ToList()
                        join b in _dataContext.Colors on a.ColorId equals b.Id select new ProductColorModel()
                        {
                            ColorCode = b.ColorCode,
                            ColorId = a.ColorId,
                            ProductId = a.ProductId,
