    ...
    public class ColorHandler : IColorHandler
    {
        ...
        public async Task<Response> Create(ColorBaseModel model)
        {
            try
            {
                ...
                var isExist = _dataContext.Colors.Any(c => c.Code == model.Code);
                if (isExist)
                    return new ResponseError(Code.NotFound, "Mã màu sắc đã tồn tại");
                #endregion

                var entity = AutoMapperUtils.AutoMap<ColorBaseModel, Domain.Entities.Color>(model);
                entity.Id = Guid.NewGuid();
                await _dataContext.Colors.AddAsync(entity);
        ...
        public async Task<Response> Update(ColorBaseModel model)
        {
            ...
            {
                #region Check is exist
                var isExist = _dataContext.Colors.Any(c => c.Code == model.Code && c.Id != model.Id);
                if (isExist)
                    return new ResponseError(Code.NotFound, "Mã màu sắc đã tồn tại");
                #endregion

                var entity = AutoMapperUtils.AutoMap<ColorBaseModel, Domain.Entities.Color>(model);
                _dataContext.Colors.Update(entity);
        ...
        public async Task<Response> Delete(List<Guid> listId)
        {
            try
            {
                ...
                {
                    name = "";
                    var entity = await _dataContext.Colors.FindAsync(item);
        ...
        public async Task<Response> Filter(ColorFilterModel filter)
        {
            try
            {
                ...
                var data = (from c in _dataContext.Colors
        ...
        public async Task<Response> GetById(Guid id)
        {
            try
            {
                var entity = await (from c in _dataContext.Colors
                                    where c.Id == id
                                    select new ColorBaseModel()
                                    {
                                        Id = c.Id,
        ...
        public async Task<Response> GetListCombobox(int count = 0, string textSearch = "")
        {
            try
            {
                var data = await (from item in _dataContext.Colors.OrderBy(x => x.Created).ThenBy(x => x.Name)
